...
without it. Its trust boundary is described in `docs/DEPLOY.md`.
...
- **Push.** Web Push with VAPID; subscriptions are validated (SSRF hardening,
  see `SECURITY.md`) and limited to ten per account.
...
`REGISTRATION_ENABLED`, `VAPID_*`, `DB_STATEMENT_TIMEOUT_MS`,
`DB_MAX_OPEN_CONNS`, `DB_CONNECT_TIMEOUT_S`, `DB_CONN_MAX_IDLE_S`,
`WS_COMPRESSION`, `DB_BACKUP_*` (see `BACKUP.md`) and the retention settings
(see `RETENTION.md`). The full list with generator commands is in
`.env.example`.
...
Next steps and priorities are in `ROADMAP.md`. Security is in `SECURITY.md`.

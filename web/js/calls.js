...
let _icePending = null; // request in flight — so credentials are not fetched twice

// warmIceServers starts fetching TURN credentials without waiting for them.
//
// It is needed because the chain used to be strictly sequential: getUserMedia
// (hundreds of milliseconds to open the microphone and get permission) → and
// ONLY THEN a trip to /turn-credentials. The extra round-trip was added to
// every call after the microphone was already open — that is, to the pause
// between the tap and the ringback tone, the one a person notices most.
function warmIceServers() {
  if (_iceCache && Date.now() < _iceCache.expiresAt) return;
  if (!token) return; // before sign-in credentials are not issued anyway
  getIceServers().catch(() => {});
}

async function getIceServers() {
  if (_iceCache && Date.now() < _iceCache.expiresAt) return _iceCache.config;
  // A second call in a row must not fetch credentials again until the first
  // request has returned.
  if (_icePending) return _icePending;
  _icePending = _fetchIceServers().finally(() => { _icePending = null; });
  return _icePending;
}
...
async function _createPC() {
  ...
  // Candidate pool: the browser starts gathering ICE without waiting for the
  // first createOffer. One pair of candidates covers a typical one-to-one call
  // and removes part of the delay before the first ringback tone.
  const pc = new RTCPeerConnection({ ...config, iceCandidatePoolSize: 1 });
...
async function startCall(video = false) {
  ...
  if (peerConnection) return;
  // The socket must be ALIVE, not merely exist. The previous check (`!ws`) let
  // a closed and reconnecting socket through: the invitation went into the
  // wsSend queue, "Calling…" hung on screen, and nothing rang for the other
  // person — right up to the reconnect. Refusing straight away is more honest.
  if (!ws || ws.readyState !== 1) {
    toast('No connection — reconnecting. Try the call again in a moment.');
    return;
  }
  callIsVideo   = !!video;
  ...
  // TURN credentials are fetched IN PARALLEL with opening the microphone, not
  // after it: getUserMedia takes hundreds of milliseconds, and the request
  // manages to return inside that window for free.
  warmIceServers();
...
async function acceptCall() {
  ...
  // The same as on the caller side: credentials are needed right after
  // getUserMedia, and fetching them sequentially means adding a round-trip to
  // the pause between "accept" and voice.
  warmIceServers();
...
async function handleCallMessage(msg) {
  if (msg.type === 'call-offer') {
    ...
    // While the phone is ringing we fetch TURN credentials. By the time the
    // person taps "accept" they are already in the cache — and no network trip
    // remains between the tap and the voice.
    warmIceServers();
    callPeer     = msg.from;

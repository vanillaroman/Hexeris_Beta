...
async function doForward(peer) {
  ...
  if (!chats[peer] && !peer.startsWith('g:')) {
    try {
      ...
      if (r.ok && typeof unmarkPeerGone === 'function') unmarkPeerGone(peer);
    } catch {}
  }

  // Confirm before sending. In a contact list one slip of the finger and the
  // message has gone to the wrong person; a forward cannot be undone, it can
  // only be deleted on your own side. So we ask, and the question names the
  // recipient by name rather than by username: usernames look alike, which is
  // exactly why the mistake is easy.
  //
  // The list is closed BEFORE the question: both modals are .hex-modal-overlay
  // with the same z-index, and the list, being lower in the DOM, would cover
  // the question. On cancel the list reopens so the person continues choosing
  // from the same place.
  if (typeof hexConfirm === 'function') {
    closeForward();
    const name = (typeof displayName === 'function') ? displayName(peer) : peer;
    const ok = await hexConfirm('Forward this message to ' + name + '?',
      { title: 'Forward', okText: 'Forward' });
    if (!ok) { openForward(); return; }
  }

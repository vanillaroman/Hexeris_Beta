...
- **Changing one's own password** (`/change-password`) — without it a user
  ...
  created here.
- **Two-factor authentication (TOTP)** — one-time codes from an authenticator
  app on top of the local password; each employee enables it themselves. With
  the second factor on, `/login` returns a **ticket** rather than a token: a
  token issued "while verification is in progress" would mean no second factor
  at all. The secret is stored encrypted with `SERVER_ENC_KEY` — in plaintext a
  database dump would cancel the protection, which the same dump does not do to
  passwords. Each code is single-use: without that, a code read over someone's
  shoulder would keep working for another minute and a half. Details and limits:
  `TWO-FACTOR.md`.
...
- **`/unfurl`** (link previews) uses the same dialler; after the redirect limit
  a 3xx response is **not** accepted; timeouts throughout. The limit counts
  **only real trips outside**: a preview served from cache does not spend quota,
  because it goes nowhere — and the limit exists precisely so that we do not
  hammer other people's sites (`../operations/LINK-PREVIEW-CACHE.md`).
...
| `/unfurl` | 40 / 10 min | user (**cache misses only**) |
...
- One key, `SERVER_ENC_KEY`. ⚠️ **A backup without it is useless**; store it
  separately (see `../operations/BACKUP.md`). Legacy plaintext is read
  transparently, and damaged ciphertext is rejected; both are covered by tests.
...
- 📏 No external penetration test has been performed. An internal audit did
  cover the authorisation and IDOR boundaries (`/history`, `/reactions`,
  edit/delete, groups — nothing found), the SSRF dialler, JWT handling (the
  signing method is fixed, so `alg=none` and algorithm confusion are
  impossible), uploads and file serving, and the rate limits. What it found,
  and the fixes, are recorded in `ROADMAP.md` and
  `../engineering/ARCHITECTURE.md` §6 and §10.

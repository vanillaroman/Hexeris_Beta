...
### If the `app` container restarts in a loop

Check its logs first — the server states the reason on one line and exits
...
| Line in the log | Cause and fix |
|---|---|
| `FATAL: UPLOAD_DIR … does not exist` | The image creates `/data/uploads` and `/data/backups`, and Docker seeds a **new named volume** from that. A **bind mount** is not seeded: if you replaced the `appdata` volume with a host directory, create those two directories inside it yourself and give the container's user access. |
| `FATAL: SERVER_ENC_KEY …` | The key does not match the one the database was encrypted with. This is a guard, not a failure — see `ENC-KEY-GUARD.md`. |
| `db ping: … connection refused` | The database has not finished starting. Compose waits for its health check, so this normally resolves itself; if it does not, look at `docker compose logs db`. |

An older volume created before the image made those directories will still be
empty. `docker compose down -v` recreates it correctly, at the cost of the data
in it.
...
In `.env`: `DB_BACKUP_ENABLED=true` — backup sets (`hexeris-*.sql.gz` plus
`hexeris-files-*.tar.gz`) are written to the `appdata` volume
(`/data/backups`). One-off run:

```bash
docker compose -f docker-compose.prod.yml exec app hexeris backup
```

Details and off-site copies: `docs/operations/BACKUP.md`. **Keep
`SERVER_ENC_KEY` somewhere separate — without it a backup is useless.**
...
### First decide which proxy to edit

It is easy to start fixing this on the wrong machine. Separating the two paths
helps:

```
AN EMPLOYEE SIGNS IN (this is what the log records):
  phone ──► nginx on the application host ──► messenger     ← EDIT HERE
  The admin site is not in this path at all.

AN OPERATOR OPENS THE ADMIN PANEL:
  browser ──► nginx on the admin site ──► nginx on the application host ──► messenger
```

Editing `/admin-api/` on the admin site does not affect the sign-in log: the
panel goes through it, employees do not. Worse, forwarding `X-Forwarded-For`
from there is **harmful** — the application takes the client address from that
header, so the operator's browser could supply any value and walk straight past
`ADMIN_ALLOWED_IPS`. That is why the sample admin-site config deliberately
overwrites the header instead of passing it through.

Asking the application itself is faster than reading configs, and leaves no
guesswork. **From the device whose address should appear in the log** (phone,
work laptop) open:

```
https://<messenger domain>/healthz?v=1
```

Look at the `client` block:

| what you see | what it means |
|---|---|
| `"ip": "127.0.0.1"`, `"x_forwarded_for": ""`, `"proxy_trusted": true` | the proxy is not passing the address on — fix its config (below) |
| `"ip"` is your real address | working as intended; the log will record the same value |
| `"proxy_trusted": false` with a non-empty `x_forwarded_for` | the header arrives but its source is not trusted — add the proxy address to `TRUSTED_PROXY_IPS` |

The `ip` field is exactly the value that will land in the sign-in log, so the
check answers the question directly rather than by inference.

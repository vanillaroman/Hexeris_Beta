...
There is one key per installation. Rotation is a separate subcommand; the
full procedure lives in `ENC-KEY-ROTATION.md`. In short:

1. Generate a new key, **keeping the old one**.
2. Stop the server and take a backup.
3. `hexeris rotate-enc-key --dry-run` — see how much work it is.
4. `hexeris rotate-enc-key --yes` — re-encrypt message bodies, two-factor
   secrets and files. The run is restartable: an interruption does not corrupt
   data.
5. Replace `SERVER_ENC_KEY` and start the server.

> Downtime for the duration of the run is unavoidable: on a running server some
> records would be written *during* rotation, with the old key, and would stay
> unreadable. Do **not** discard the old key — it is still required for backups
> taken before the rotation.

Separately: the server **will not start** if the key does not match the one the
database was encrypted with. That is expected behaviour, not a broken restore.
The procedure, including how to confirm a deliberate key change, is in
`docs/operations/ENC-KEY-GUARD.md`. The practical consequence for scenarios A
and B: restore the secrets **before** the first start, or that first start will
stop at this check.
...
- **`SERVER_ENC_KEY` rotation** has been exercised on a database of ~14 000
  messages, but not at production volumes and not against a multi-gigabyte
  upload directory: the run time for that case was never timed.
- **A single machine.** Everything above is about recovery rather than high
  ...
  unavoidable (see `ROADMAP.md`, SPOF).

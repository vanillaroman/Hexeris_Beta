...
`docs/engineering/ARCHITECTURE.md` (topology, ports, schema, data flows),
`docs/security/SECURITY.md` (security model), `docs/operations/BACKUP.md` and `docs/operations/RETENTION.md`.
...
- ✅ Chat-list management: pin, mute, archive and "delete for me". Everything
  ...
  press on a phone.
- ✅ The interface was brought to one system: a single surface for every
  dialog (there had been four widths and two incompatible looks), multi-step
  dialogs no longer shift between steps, and the light theme moved to a warm
  beige. Readability is measured rather than eyeballed: a sweep over 16 desktop
  states and 4 phone states in both themes (`tests/ui/uitest_contrast.js`).
- ✅ Accessibility: a live region announces incoming messages, focus moves into
  dialogs and returns where it came from, and Tab is trapped inside them.
- ✅ Attachments panel per conversation (media, files, voice) backed by an
  indexed `media_type` column rather than a scan.
- ✅ Security: JWT with bcrypt, encryption at rest (bodies AES-256-GCM, files
  ...
  without internet access. Details in `docs/security/SECURITY.md`.
...
- ✅ Performance work: batched message writes (×6 throughput), Postgres tuning,
  ...
  (`docs/engineering/ARCHITECTURE.md §6`, measurements in §4).
...
- ✅ **`/history` is optimised, with the cause established rather than
  ...
  `docs/engineering/ARCHITECTURE.md §6`.
...
- ✅ **A database outage no longer hangs senders**: `ack=failed` in 0.0 s
  ...
  down from 12.3 s to 4.1 s (`docs/engineering/ARCHITECTURE.md §10`).
...
- ✅ **Off-site copying is implemented** through `DB_BACKUP_OFFSITE_CMD` (any
  ...
  (`docs/operations/DISASTER-RECOVERY.md §1.1`).
- ✅ **A disaster-recovery document** — `docs/operations/DISASTER-RECOVERY.md`: three
...
- **Recovery after a database restart is not instant.** `database/sql` discards
  ...
  that it is bounded by `DB_CONN_MAX_IDLE_S` (`docs/engineering/ARCHITECTURE.md §10`).
- **`delivered=true` means "queued for the socket", not "the client received
  ...
  (`docs/engineering/ARCHITECTURE.md §4.1`).
...
- **Files are reachable by link to any authenticated user** (a capability URL):
  ...
  working. See `docs/security/SECURITY.md`.
...
> query plan are in `docs/engineering/ARCHITECTURE.md §10`.
...
- **D. Security** — can the claimed level be demonstrated? *(`docs/security/SECURITY.md`
...
| `docs/engineering/ARCHITECTURE.md` | Topology, ports, API, schema, data flows, failure points |
| `docs/security/SECURITY.md` | Headers, authorisation, SSRF, rate limits, encryption, tests |
| `docs/operations/BACKUP.md` | Database and file backups, retention, restore procedure |
| `docs/operations/DISASTER-RECOVERY.md` | Recovery scenarios, RTO/RPO, what remains unmeasured |
| `docs/operations/RETENTION.md` | The retention janitor (disabled by default) |
| `docs/operations/DEPLOY.md` | Installation, production, the admin panel host |
